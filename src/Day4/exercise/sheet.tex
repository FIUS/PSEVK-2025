...
\begin{enumerate}
    \item Erstelle eine neue Klasse (wieder mit neuer Datei) namens \texttt{Grillgut}. Sie soll einen Grillguttyp (\texttt{String}, z.B. Currywurst), eine minimale Garstufe (\texttt{int}), eine maximale Garstufe (\texttt{int}), eine momentane Garstufe (\texttt{int}), eine Hitzetoleranz (\texttt{int}) und einen verbrannt (\texttt{Boolean}) Status haben. Wenn das Grillgut einmal mit einer Hitze größer der Hitzetoleranz gegrillt wird, dann ist es sofort verbrannt.
    ...
    \item Erstelle eine \texttt{grillen} Funktion, die Hitze als Parameter entgegennimmt und das Grillgut entsprechend grillt.
