...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item\pause Funktionen können Werte zurück geben
        ...
        \end{minted}
    \item\pause Datentyp des \texttt{return} Wert's muss im Funktionskopf stehen
    \item\pause Wenn kein Rückgabewert $\rightarrow$ \texttt{void}
    \end{itemize}
\end{frame}
...
\begin{itemize}
    \item<1-> Funktionen können Werte zurückgeben
    \item<2-> z.B. eine Zahl:
...
\end{minted}
    ...
    \item<3-> Datentyp des \texttt{return}-Werts muss im Funktionskopf stehen

    \begin{tikzpicture}[remember picture, overlay]
      \draw[->, red, thick]
        ([yshift=0.4ex]pic cs:rettyp)
        -- ++(1.8, 0)
        node[right] {\scriptsize \texttt{int} = Rückgabetyp};
    \end{tikzpicture}

    \item<4-> Wenn kein Rückgabewert $\rightarrow$ \texttt{void}
\end{itemize}
